...
import { useGetStatisticYearlyQuery } from "@/redux/features/wallet/walletApi";
...
const ThisYear = () => {
  ...
  const { data: transactionsByYear, isFetching } = useGetStatisticYearlyQuery(
  ...
  const processChartData = () => {
    if (!transactionsByYear?.data?.transactionsByYear) {
      return [];
    }
    let maxAmount = 0;
    transactionsByYear.data.transactionsByYear.forEach((year) => {
      maxAmount = Math.max(maxAmount, year.totalIncome, year.totalExpense);
    });
    return transactionsByYear.data.transactionsByYear.map((item) => {
      const formattedDate = formatChartDate(new Date(item.year), "year");
      return {
        year: formattedDate,
